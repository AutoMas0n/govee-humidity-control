...
- **THEN** the system SHALL log a warning and skip the reading cycle
...
- **WHEN** manufacturer data contains a valid temperature reading in the Govee H5179 format (little-endian signed short at byte offset 4 with tenths at byte offset 5)
...
- **THEN** the system SHALL return humidity as an integer percentage
...
- **WHEN** manufacturer data is shorter than expected
...
- **THEN** the system SHALL use passive BLE advertisement scanning only

### Requirement: Read on a fixed interval

The system SHALL read humidity on a configurable interval.

#### Scenario: Cycle executes on schedule

- **WHEN** the configured interval elapses (default: every 15 minutes)
